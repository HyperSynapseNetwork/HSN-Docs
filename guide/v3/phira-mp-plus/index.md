...
| 分组 | 命令 | 说明 |
|------|------|------|
| 通用 | `help`, `exit`, `status`, `version`, `check-config`, `doctor` | 帮助/退出/状态/版本/诊断 |
| WASM 插件 | `plugin list/enable/disable/info/reload/call` | 插件管理 |
| 用户 | `users`, `kick`, `admin-id`, `broadcast` | 用户管理和消息 |
| 房间 | `rooms`, `room info/start/host/set/lock/cycle/close/...` | 房间管理子命令 |
| 黑名单 | `ban`, `unban`, `banlist`, `ip-history` | 封禁管理 |
| 扩展数据 | `extension list/get` | 扩展字段 |
| 运维 | `wal inspect`, `dead-letter list/replay`, `config reload` | 运维工具 |
