...
<img src="https://raw.githubusercontent.com/HyperSynapseNetwork/HSN-Docs/refs/heads/main/guide/v3/phira-mp-plus/docs/logo/phira-mp-plus-banner.png" width="60%" alt="Phira-mp+ banner" />

**Phira-mp+（PMP）** — 基于 [phira-mp](https://github.com/HyperSynapseNetwork/phira-mp) 的高性能 Phira 多人游戏服务端 · Rust / WASM 插件 / WAL 先行 / Actor 模型

<br/>

[![License: Apache 2.0](https://img.shields.io/badge/License-Apache%202.0-blue.svg)](https://github.com/HyperSynapseNetwork/phira-mp/LICENSE)
...
- **丰富的拓展功能**：PMP相对于原版Phira-mp提供了大量新功能以优化用户与运维体验，详见[功能总览](https://docs.phira.htadiy.com/guide/v3/phira-mp-plus/features.html)与[CLI 手册](https://docs.phira.htadiy.com/guide/v3/phira-mp-plus/cli.html)

> [!NOTE]
> 想深入看实现？见[架构文档](https://docs.phira.htadiy.com/guide/v3/phira-mp-plus/development/architecture.html)。

## 文档

| 分类 | 文档 |
|------|------|
| **功能总览** | [PMP 相对 Phira-mp 新增功能](https://docs.phira.htadiy.com/guide/v3/phira-mp-plus/features.html) |
| **部署与运维** | [部署/配置/运维](https://docs.phira.htadiy.com/guide/v3/phira-mp-plus/deployment.html) · [配置 JSON Schema](https://docs.phira.htadiy.com/guide/v3/phira-mp-plus/operations/config-schema.json) |
| **对外 API** | [HTTP/SSE/WS · 插件 API · 能力表 · OpenUDS](https://docs.phira.htadiy.com/guide/v3/phira-mp-plus/api.html) |
| **CLI 手册** | [CLI 命令参考](https://docs.phira.htadiy.com/guide/v3/phira-mp-plus/cli.html)（含[基准测试](https://docs.phira.htadiy.com/guide/v3/phira-mp-plus/cli.html#基准测试)） |
| **插件开发** | [插件开发指南](https://docs.phira.htadiy.com/guide/v3/phira-mp-plus/plugin-dev.html)（含 WIT ABI、示例） |
| **开发** | [架构](https://docs.phira.htadiy.com/guide/v3/phira-mp-plus/development/architecture.html) · [测试指南](https://docs.phira.htadiy.com/guide/v3/phira-mp-plus/development/testing.html) · [CLI 错误码 (EN)](https://docs.phira.htadiy.com/guide/v3/phira-mp-plus/development/error-codes.en.html) |
...
配置加载规则：默认读取 `server_config.yml`，也可通过 `--config <FILE>` 指定；配置文件缺失时使用内置默认值，配置文件存在但格式、字段名或取值无效时拒绝启动。只有用户显式提供的命令行参数才覆盖 YAML，避免 CLI 默认值意外覆盖配置文件。完整说明见 [docs/deployment.html](https://docs.phira.htadiy.com/guide/v3/phira-mp-plus/deployment.html)。
...
空载模式仅改变非关键后台活动的调度偏好，不会暂停权威持久化或可靠插件事件。更多配置项见 [docs/deployment.html](https://docs.phira.htadiy.com/guide/v3/phira-mp-plus/deployment.html)。
```
...
Phira-mp+ 整体采用 **Apache License, Version 2.0** — 详见 [LICENSE](https://github.com/HyperSynapseNetwork/Phira-mp-plus/LICENSE)。

协议层（`phira-mp-common`、`phira-mp-macros`）基于 [phira-mp](https://github.com/TeamFlos/phira-mp) 衍生，同样为 Apache-2.0。

完整的版权归属和第三方依赖许可证声明见 [THIRDPARTY_LICENSES](https://github.com/HyperSynapseNetwork/Phira-mp-plus/THIRDPARTY_LICENSES)。

## 致谢

感谢 [TeamFlos](https://github.com/TeamFlos) 开发和维护 Phira、phira-mp 项目，以及 [tphira-mp](https://github.com/Pimeng/tphira-mp) 与 [jphira-mp](https://github.com/lRENyaaa/jphira-mp) 提供的实现思路，还有所有支持本项目的用户。版权归属与第三方依赖许可证见 [THIRDPARTY_LICENSES](https://github.com/HyperSynapseNetwork/Phira-mp-plus/THIRDPARTY_LICENSES)。
